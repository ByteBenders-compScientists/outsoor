...
interface DashboardPageClientProps { user: User }

export function DashboardPageClient({ user }: DashboardPageClientProps) {
  const pathname = usePathname()

  const renderContent = () => {
    if (pathname === "/dashboard")                          return <DashboardChatInterface user={user} />
    if (pathname === "/dashboard/apis")                    return <ApiTokensMain user={user} />
    if (pathname === "/dashboard/billing")                 return <BillingMain user={user} />
    if (pathname === "/dashboard/settings")                return <SettingsMain user={user} />
    if (pathname === "/dashboard/security")                return <SecurityMain user={user} />
    if (pathname === "/dashboard/notifications")           return <NotificationsMain user={user} />
    if (pathname === "/dashboard/models")                  return <ModelsMain user={user} />
    if (pathname === "/dashboard/models/docs")             return <ModelsDocsIndex user={user} />
    if (pathname === "/dashboard/team")                    return <TeamPage user={user} />
